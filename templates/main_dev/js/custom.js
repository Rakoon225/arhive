...
function product_popup() {
  $('.js-modal-show-product').on('click', function () {
    var parrentDiv = $(this).parents('.eq-selected, .equipment-v2__box--selected');

    let currentModal = $(this).attr('href');
    let currentProduct = $(this).attr('data-id');
    let currentComplectation = $('span.eq-selected__name, .quipment-v2-card__title--active', parrentDiv).html();
    let currentPrice = $('p.eq-selected__price, .quipment-v2-card__price--active', parrentDiv).html();
